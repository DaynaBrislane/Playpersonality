    ...
    async function handleSubmit() {
      ...
      if (res.ok) {
        if (window.location.hostname.includes('playpersonality')) {
          window.location.href = '/collaboration-profiles.html';
        } else {
...
export default async function middleware(request) {
  ...
  if (match && match[1] === hashToken('cosmiclatte')) {
    // On playpersonality, redirect root to collaboration profiles
    if (url.pathname === '/' && url.hostname.includes('playpersonality')) {
      return Response.redirect(new URL('/collaboration-profiles.html', request.url), 302);
    }
